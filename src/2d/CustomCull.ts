...
export class CustomCull {
    ...
    cull(rect: Rectangle, skipUpdate = false): this {
        ...
        this._targetList.forEach((target) => {
            ...
            if (this._recursive) {
                ...
                if (target[this._toggle] && (tempRect.width < minPixelWidth || tempRect.height < minPixelHeight)) {
    ...
    isCullInProgress(): boolean {
        return this._cullInProgress;
    }
    ...
    protected cullRecursive(rect: Rectangle, displayObject: DisplayObject, skipUpdate?: boolean): void {
        ...
        if (displayObject[this._toggle] && (bounds.width < minPixelWidth || bounds.height < minPixelHeight)) {
            displayObject[this._toggle] = false;
        }
        ...
        if (!fullyVisible && displayObject[this._toggle] && children.length) {
            for (let i = 0, j = children.length; i < j; i++) {
                this.cullRecursive(rect, children[i]);
            }
        } else if (displayObject[this._toggle] && children.length) {
            for (let i = 0, j = children.length; i < j; i++) {
                this.cullBySize(rect, children[i]);
            }
        }
    }
    ...
    protected cullBySize(rect: Rectangle, displayObject: DisplayObject, skipUpdate?: boolean): void {
        const bounds = skipUpdate
            ? displayObject._bounds.getRectangle(tempRect)
            : displayObject.getBounds(true, tempRect);

        displayObject[this._toggle] = bounds.width >= minPixelWidth && bounds.height >= minPixelHeight;

        const children = (displayObject as Container).children || [];

        if (displayObject[this._toggle] && children.length) {
            for (let i = 0, j = children.length; i < j; i++) {
                this.cullBySize(rect, children[i]);
            }
        }
